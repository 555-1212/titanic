...
Model$set("public", "showModelError", function() {
  plot(private$rf_model, ylim = c(0, 0.36))
  legend('topright',
         colnames(private$rf_model$err.rate),
         col = 1:3,
         fill = 1:3)
})

## Variable importance by plotting the mean decrease in Gini calculated across all trees.
Model$set("public", "showModelImportance", function() {
# Get importance
importance    <- importance(private$rf_model)
